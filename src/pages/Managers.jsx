...
import { computeManagers, faNum, fmtPercent } from '../lib/fipiran'

function ManagerNameCell({ row }) {
  return (
...
const MANAGER_COLUMNS = [
  {
    ...
    label: 'نام مدیر',
    align: 'start',
    render: (row) => <ManagerNameCell row={row} />,
  },
  {
    key: 'aum',
    label: 'دارایی تحت مدیریت (هزار میلیارد تومان)',
    sortVal: (row) => row.aumTBT,
    render: (row) => <AumCell value={row.aumTBT} />,
  },
  {
    key: 'count',
    label: 'تعداد صندوق',
  ...
  {
    key: 'flowPct',
    label: 'درصد تغییرات دارایی',
    sortVal: (row) => row.flowPct,
    render: (row) => <FlowPctCell value={row.flowPct} />,
  },
]

const GOOD_SORT_KEYS = ['aum', 'count', 'years', 'flow', 'flowPct']
...
export default function Managers() {
  ...
  return (
    <FundsPageLayout
      ...
      title="مدیران"
      highlight="صندوق‌ها"
      subtitle="رتبه‌بندی شرکت‌های مدیریت دارایی بر اساس حجم و عملکرد"
      loading={loading}
      startDate={startDate}
      endDate={endDate}
      startISO={startISO}
    ...
    >
      ...
      <p className="text-center text-text-muted text-xs font-dana mt-5 leading-relaxed" style={{ fontWeight: 600 }}>
        منبع: فیپیران · برندهای یکسان با انواع مختلف نهاد (سبدگردان، مشاور، کارگزاری) در یک گروه ادغام شده‌اند · سابقه از تاریخ تأسیس اولین صندوق
      </p>
    </FundsPageLayout>
  )
}
